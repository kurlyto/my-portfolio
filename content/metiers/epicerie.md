---
titre: "Agent IA pour commerce de proximité : stock, DLC, avis"
description: "Un agent IA qui surveille les dates limites et prépare les commandes fournisseurs. Il lance les promotions, répond aux avis Google et pense à vos habitués."
h1: "Un agent IA pour votre commerce de proximité"
date: 2026-09-04
mot_cle: "agent ia commerce"
mots_cles: ["agent IA commerce de proximité", "gestion de stock", "dates limites", "commandes fournisseurs", "anti-gaspillage"]
lire_aussi: ["/blog/quel-agent-ia-pour-votre-metier"]
---
Un agent IA pour commerce de proximité est un assistant logiciel qui suit vos produits depuis le carton du livreur jusqu'au ticket de caisse pendant que vous servez vos clients. Il tient le stock et les dates limites, prépare les promotions et les commandes fournisseurs et répond aux messages comme aux avis Google. Il applique seul les règles que vous lui avez fixées et vient vous chercher pour tout le reste.

Il ne conseillera jamais un fromage ou un roman aussi bien que vous. Son terrain commence derrière le comptoir et cette page vaut pour une épicerie, une cave ou une fromagerie autant que pour un fleuriste, une librairie ou une boutique de décoration.

<div class="encadre">
<p><strong>En bref</strong></p>
<p>L'agent lit vos bons de livraison, tient le stock et surveille les dates limites. Trois jours avant une date il prépare une promotion ou un panier anti-gaspi avec le post qui l'annonce. Il monte aussi la commande fournisseur, répond aux avis Google et prévient vos habitués quand un arrivage les concerne.</p>
<p>Trois gestes lui restent interdits : écrire dans votre caisse, envoyer une commande que vous n'avez pas validée et écrire à des clients qui n'ont pas donné leur accord.</p>
</div>

## La semaine d'un commerçant

Conseiller une bouteille pour un dîner ou mettre de côté le dernier bouquet de pivoines pour une habituée fait partie du plaisir du métier et ce qui pèse se passe plutôt dans le dos du comptoir.

Prenons un jeudi ordinaire dans une boutique de quartier. Le livreur a déposé six cartons à l'ouverture et personne n'a eu le temps de pointer le bon de livraison. Sur Instagram une cliente demande si le comté 24 mois est revenu pendant qu'un avis à deux étoiles attend depuis mardi sur Google. La fiche Google annonce encore les horaires d'été, le planning de la semaine prochaine n'est pas fait et quatre yaourts fermiers arrivent à leur date samedi au fond de la vitrine.

Aucune de ces tâches ne demande plus de cinq minutes et chacune se paie au prix fort quand on l'oublie.

## Les tâches confiées à l'agent

Imaginez un employé qui ne quitterait jamais l'arrière-boutique et qui garderait en tête chaque carton entré et chaque article sorti. Si le mot vous paraît encore flou, vous trouverez [ce qu'est un agent IA](/blog/agent-ia-definition) dans un article à part. Pour un commerce les missions de l'agent se rangent presque toujours en trois familles.

- **La gestion de la boutique.** Le stock, les seuils de réassort et les dates limites sont suivis chaque jour et la commande fournisseur arrive prête à relire.
- **Le marketing du quotidien.** L'agent prépare les promotions et les posts et tient la fiche Google à jour. Il répond aux avis et aux questions d'horaires, d'arrivages et de disponibilité qui tombent sur Instagram.
- **L'administratif.** Il range les factures fournisseurs chaque semaine et propose le planning du mois d'après les contraintes de chacun.

Le tout se résume chaque matin dans un seul message que vous lisez avant de lever le rideau. Rien n'oblige à tout lui confier le premier jour et je commence presque toujours par **le stock et les dates** parce que tout le reste s'appuie dessus.

## Le stock et les dates limites

### La livraison devient une fiche propre

Quand le livreur passe, vous photographiez le bon de livraison ou vous dictez l'arrivage à l'agent comme à un employé. Il reconnaît chaque ligne par son code-barres, ajoute les quantités au stock et note la date imprimée sur le lot. Tout démarre par un fichier produits bien rangé, parce qu'un pot noté « miel foncé petit » dans un tableau et enregistré autrement dans la caisse devient deux produits que rien ne recoupe plus.

<figure><img src="/images/metiers/epicerie-rayons.webp" alt="L'intérieur d'une épicerie fine aux rayons chargés jusqu'au plafond, trois personnes au comptoir" width="1400" height="932" loading="lazy"><figcaption>L'épicerie Izrael à Paris en 2012. Des centaines de références et autant de dates limites à surveiller.<span class="credit">Photo : Piero d'Houin dit Inocybe, <a href="https://creativecommons.org/licenses/by-sa/3.0">CC BY-SA 3.0</a>, via <a href="https://commons.wikimedia.org/wiki/File:%C3%89picerie_Izrael_en_2012.jpg">Wikimedia Commons</a></span></figcaption></figure>

### Le stock se tient entre deux clients

Les ventes descendent du stock sans vous parce que l'agent **lit la caisse sans jamais y écrire**. Je tiens à ce qu'elle reste la source de vérité parce qu'un outil qui irait y corriger un prix dans votre dos ruinerait votre confiance en une journée. La casse et les dégustations ne passent par aucun ticket mais il suffit d'écrire « deux pots cassés » à l'agent pour que le compte reste juste.

### Deux dates qui n'ont pas le même poids

La date limite de consommation se reconnaît à la mention « à consommer jusqu'au ». Vendre le produit après cette date est interdit et Service-Public chiffre l'amende à [1 500 € par produit périmé](https://www.service-public.gouv.fr/particuliers/vosdroits/F10990) en cas de simple négligence. La date de durabilité minimale s'annonce par « à consommer de préférence avant » et ne parle que de goût et de texture, si bien qu'un paquet de pâtes qui l'a dépassée peut rester en rayon.

Un commerce sans produits frais a lui aussi ses échéances. Le fleuriste compte les jours depuis l'arrivage des roses et le caviste sait que le beaujolais nouveau se vend mal en février.

Les rappels de produits suivent le même chemin. Les avis de [Rappel Conso](https://rappel.conso.gouv.fr/) existent en [données ouvertes classées par code-barres](https://data.economie.gouv.fr/explore/dataset/rappelconso-v2-gtin-trie/information/) et l'agent les compare chaque matin à vos fiches produits.

## Les produits qui approchent de leur date

C'est ici que l'agent gagne sa place à mon avis, parce qu'une alerte seule n'a jamais sauvé un yaourt. Trois jours avant une DLC il cherche une porte de sortie pour le produit et vous la propose toute montée.

Vous fixez les règles une fois. Vous décidez par exemple qu'un produit frais passe à −20 % trois jours avant sa date, qu'il rejoint un panier anti-gaspi la veille et qu'il quitte le rayon le dernier soir. L'agent prépare alors l'étiquette à imprimer et rédige le post qui annonce l'offre.

<figure>
<div class="message">
<div class="expediteur"><span>Assistant de la boutique</span><span>7:32</span></div>
<div class="ligne"><strong>Dates à surveiller</strong></div>
<div class="ligne">Yaourts fermiers, 4 pots, DLC samedi. Je propose −20 % dès ce matin. L'étiquette et le post sont prêts.</div>
<div class="ligne">Burrata, 3 pièces, DLC vendredi. Deux habituées inscrites à vos messages en prennent chaque semaine. Je peux les prévenir.</div>
<div class="ligne">Sablés au beurre, 6 boîtes, date de durabilité dépassée dimanche. Ils restent vendables. Je suggère un panier anti-gaspi avec le thé en vrac.</div>
<div class="actions"><span>Appliquer</span><span>Modifier</span><span>Ignorer</span></div>
</div>
<figcaption>Exemple d'alerte sur les dates. L'agent propose une sortie pour chaque produit et attend votre feu vert. Les produits et les chiffres sont fictifs.</figcaption>
</figure>

Au début je conseille de valider chaque proposition d'un mot. Quand une règle a fait ses preuves, vous pouvez laisser l'agent dérouler seul tout ce qui ne touche pas la caisse. Il change alors le prix de la vitrine en ligne et publie le post puis retire le produit du site le dernier soir.

<figure>
<ol class="flux">
<li><strong>Livraison</strong>photo du bon, dates lues</li>
<li><strong>Stock</strong>la caisse est lue, jamais modifiée</li>
<li><strong>Alerte</strong>trois jours avant la date</li>
<li><strong>Promotion et post</strong>étiquette, site, Instagram</li>
<li><strong>Vente ou retrait</strong>bilan envoyé le dernier soir</li>
</ol>
<figcaption>Le parcours d'un produit frais, de la livraison à la vente. Vous fixez les règles une fois et l'agent les déroule.</figcaption>
</figure>

L'agent envoie chaque semaine le bilan de ce que ces règles ont sauvé et ce bilan sert à ajuster la commande suivante, parce qu'un produit bradé tous les quinze jours est d'abord un produit commandé en trop.

## Les promotions et les réseaux sociaux

L'agent repère aussi l'article qui dort depuis trois semaines et vous suggère un lot avec un produit qui marche ou une place plus près de la caisse.

### Ce que la loi impose sur les promotions

Le prix barré doit correspondre [au prix le plus bas pratiqué au cours des 30 jours qui précèdent](https://www.service-public.gouv.fr/particuliers/actualites/A15721) et la règle vaut en magasin comme en ligne. Service-Public précise qu'elle ne s'applique pas aux produits périssables menacés d'une altération rapide, ce qui peut concerner un yaourt à deux jours de sa date mais sûrement pas un bocal de sauce. L'agent garde donc la preuve du prix de référence de chaque article.

Le vocabulaire compte aussi. En dehors des deux périodes légales, [l'emploi du mot « soldes » est interdit](https://entreprendre.service-public.gouv.fr/vosdroits/F24037) et son usage illicite peut coûter 15 000 € à un entrepreneur individuel. L'agent écrit donc « promotion » sur vos affiches de novembre.

Un libraire connaît une contrainte de plus puisque le prix d'un livre neuf est fixé par l'éditeur et que [le rabais en magasin ne peut pas dépasser 5 %](https://entreprendre.service-public.gouv.fr/vosdroits/F22713). Un agent bien réglé ne proposera donc jamais −20 % sur un roman qui dort et cherchera plutôt une table thématique.

### Une photo d'arrivage devient un post

Vous envoyez à l'agent la photo des cèpes du matin ou du carton de la rentrée littéraire et il vous renvoie un post rédigé dans votre ton avec le prix et les horaires du jour. Vous le publiez d'un mot et le même arrivage met à jour la vitrine en ligne.

### La fiche Google reste juste

Google permet de déclarer des [horaires d'ouverture exceptionnels](https://support.google.com/business/answer/6303076?hl=fr) pour un jour férié ou une courte fermeture sans toucher aux horaires habituels. L'agent connaît le calendrier et vous pose la question dix jours avant chaque jour férié puis met la fiche à jour selon votre réponse.

## Les habitués et les commandes par message

L'agent peut tenir le fichier de vos habitués aussi proprement que celui des produits. Il note qui a demandé à être prévenu du retour de la truffe, du prochain tome d'une série ou des premières pivoines de la saison. Quand le bon de livraison contient le produit attendu, il prépare le message pour ces clients-là et pour eux seuls. C'est la même mécanique que [les relances d'un salon de coiffure](/metiers/coiffeur) qui n'écrit qu'aux clientes dont la couleur a six semaines.

<figure>
<div class="message">
<div class="expediteur"><span>Assistant de la boutique</span><span>9:12</span></div>
<div class="ligne"><strong>Livraison lue</strong></div>
<div class="ligne">1 écart : 6 boîtes de sablés livrées pour 12 commandées.</div>
<div class="ligne">Le comté 24 mois est revenu. 5 clients avaient demandé à être prévenus et leur message est prêt.</div>
<div class="ligne">Le post Instagram de l'arrivage est prêt avec votre photo de ce matin.</div>
<div class="actions"><span>Prévenir les 5 clients</span><span>Voir le post</span><span>Signaler l'écart</span></div>
</div>
<figcaption>Exemple de message reçu après la photo d'un bon de livraison. Les produits et les quantités sont fictifs.</figcaption>
</figure>

Les commandes par message suivent la même logique. Un client écrit sur Instagram qu'il voudrait un plateau de fromages pour huit samedi et l'agent vérifie le stock avant de vous soumettre la demande avec un créneau de retrait. Une fois que vous avez dit oui, il confirme au client et vous obtenez un click and collect sans site marchand.

## Les fournisseurs et le planning

L'agent peut tout préparer et c'est déjà l'essentiel du travail. Quand un produit passe sous son seuil, l'agent regarde ce qui s'est vendu les dernières semaines et tient compte du jour de passage du fournisseur comme du minimum de commande. Sur [les agents que je construis](/agents) aucun n'envoie seul une commande parce qu'elle engage votre trésorerie et qu'un seuil mal réglé peut vous livrer quarante kilos de farine la veille de vos congés.

Depuis le 1er septembre 2026 [toutes les entreprises doivent être en mesure de recevoir des factures électroniques](https://entreprendre.service-public.gouv.fr/actualites/A18953) en passant par une plateforme agréée. L'agent y récupère vos factures chaque semaine, les rapproche des bons de livraison et les range dans votre outil de comptabilité.

Pour le planning vous lui donnez les contraintes de chacun. Il propose le mois suivant en respectant les règles que vous lui avez apprises, à commencer par le [repos quotidien d'au moins 11 heures consécutives](https://www.service-public.gouv.fr/particuliers/vosdroits/F990) entre deux journées de travail. Vous validez et il l'envoie à l'équipe.

## Les avis Google

Un avis sans réponse donne l'image d'une boutique où personne ne décroche. L'agent rédige une réponse à chaque avis dans votre ton et publie seul les remerciements si vous l'y autorisez. Dès qu'une note tombe sous trois étoiles il vous montre l'avis avec un brouillon et il attend, parce qu'une réponse à un client mécontent porte votre nom.

## Les lignes rouges

Plusieurs des gestes qui suivent sont techniquement à la portée d'un agent et c'est pourquoi je les lui interdis.

### Toucher à la caisse ou aux prix

Le prix affiché doit être indiqué toutes taxes comprises et de façon lisible pour que le client le connaisse [sans avoir à le demander](https://entreprendre.service-public.gouv.fr/vosdroits/F34344). L'agent prépare donc les étiquettes et vous seul changez les prix.

### Décider qu'un produit est encore vendable

Un agent lit des dates et des quantités mais il ne voit ni le sachet gonflé ni la vitrine réfrigérée tombée en panne dans la nuit. La page sur [l'agent IA pour restaurant](/metiers/restaurateur) détaille ce point côté cuisine. Retirer un produit périmé du rayon reste votre obligation et aucune alerte ne vous en décharge.

### Improviser sur les allergènes ou sur l'alcool

Dès que vous vendez à la coupe ou en vrac, la présence d'allergènes doit être [écrite à proximité du produit](https://entreprendre.service-public.gouv.fr/vosdroits/F32192). L'agent recopie donc votre fiche produit ou l'étiquette du fabricant et ne déduit jamais une composition du nom d'un produit. La vente d'alcool à distance [est considérée comme de la vente à emporter](https://www.drogues.gouv.fr/en-matiere-dalcool) si bien que l'agent ne confirme jamais seul une commande de vin et que la vérification de l'âge vous revient au retrait.

### Écrire à tout votre fichier clients

La CNIL pose une règle simple : un SMS ou un mail publicitaire adressé à un particulier suppose qu'il ait [donné son accord avant](https://www.cnil.fr/fr/la-prospection-commerciale-par-courrier-electronique-sms-mms-et-automate-dappel), par une case qu'il a cochée lui-même. L'agent n'écrit donc qu'aux clients dont l'accord est noté dans le fichier et retire sans discuter ceux qui se désinscrivent.

### Fabriquer ou trier des avis

Émettre un faux avis ou modifier un vrai est une [pratique commerciale déloyale](https://www.service-public.gouv.fr/particuliers/actualites/A15721) qui expose à deux ans de prison et 300 000 € d'amende. Google interdit de son côté [d'offrir une remise en échange d'un avis](https://support.google.com/contributionpolicy/answer/7400114?hl=fr) et de ne solliciter que les clients contents. L'agent ne proposera donc jamais dix points de fidélité contre cinq étoiles.

### Se faire passer pour vous

Le [règlement européen sur l'IA](https://digital-strategy.ec.europa.eu/fr/policies/regulatory-framework-ai) s'applique depuis le 2 août 2026 et son [article 50](https://ai-act-service-desk.ec.europa.eu/fr/ai-act/article-50) impose d'informer toute personne qui discute avec une IA. Quand l'agent répond à la question du comté sur Instagram, il se présente comme l'assistant de la boutique et ne signe jamais de votre prénom.

## Comment ça se passe ?

Tout commence par un audit gratuit pendant lequel on cherche ensemble où filent vos heures et ce qui mérite d'être confié.

Je construis ensuite l'agent sur les outils que vous utilisez déjà, en commençant par une seule brique. Son prix dépend des tâches confiées et se fixe après l'audit. Le premier mois est gratuit et sans engagement. Si l'agent ne vous a pas rendu d'heures au bout de ces quatre semaines, vous arrêtez là.

Pour savoir par quelle tâche commencer chez vous, [Nate vous répond en deux minutes](/agents?chat=1&metier=epicerie).
