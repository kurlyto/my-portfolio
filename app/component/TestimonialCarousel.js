...
export default function TestimonialCarousel() {
  ...
  return (
    ...
    >
      ...
      <div className="relative bg-[#faf8f5] text-black rounded-3xl border-2 border-accent/35 shadow-[0_20px_50px_-20px_rgba(255,107,53,0.25)] p-8 md:p-10 min-h-[340px] flex flex-col overflow-hidden">
        {/* Guillemet en filigrane : donne du caractere a la carte sans
            concurrencer le texte. */}
        <span
          aria-hidden
          className="absolute -top-10 right-4 text-[170px] leading-none font-display text-accent/[0.10] select-none"
        >
          &rdquo;
        </span>
        ...
        <AnimatePresence mode="wait">
          ...
          >
            <Stars />
            <p className="mt-4 font-display text-xl md:text-[1.6rem] leading-snug font-semibold text-balance">
              {current.quote}
            </p>
      ...
      <div className="mt-5 flex items-center justify-center gap-3">
        ...
        {/* Le trait visible fait 2px de haut : joli, mais intouchable au doigt
            (mesure du 08/09 : cibles de 12x2 px). Le bouton porte donc un
            padding transparent qui lui donne une vraie surface tactile
            (>= 24x24, recommandation WCAG 2.5.8) sans changer le dessin. */}
            ...
            >
              <span
                className={`block h-0.5 rounded-full transition-all duration-300 ${
                  i === index ? "w-7 bg-accent" : "w-3 bg-black/15 hover:bg-black/30"
                }`}
              />
            </button>
          ))}
